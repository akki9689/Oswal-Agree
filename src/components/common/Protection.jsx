...
export const Protection = ({ data, parseText }) => {
  return (
      ...
      <div className="mx-auto xl:w-10/12 mmd:w-11/12 w-10/12 flex flex-col mmd:flex-row mmd:gap-x-16 mmd:items-start">
        ...
        <div className="relative flex-1 mmd:order-1 order-2 mmd:mt-36">
          <img
            src={oneside}
            alt="First image"
            className="w-2/3 mmd:w-2/3 mb-32 h-[40vh] rounded-tl-3xl rounded-tr-3xl"
          />
          <img
            src={secondside}
            alt="Second image"
            className="absolute h-[40vh] bottom-0 bg-white-shade-100 p-2 right-0 w-2/3 mmd:w-2/3 mb-32 rounded-tl-3xl rounded-tr-3xl transform translate-x-[-20px] translate-y-[180px]"
            style={{ zIndex: 1 }}
          />
        </div>
          ...
          <div className="p-4 mt-8">
            {data.map(item => (
              <div
                ...
                className="font-open-sans leading-[1.8rem] text-justify"
              />
            ))}
          </div>
        </div>
